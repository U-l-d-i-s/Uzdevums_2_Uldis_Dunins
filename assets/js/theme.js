$(document).ready(() =>{
  ...
  $('.mouse-event').on('click', function(){
    if($('.visibility-js').is(':visible')){
      $('.visibility-js').hide('100');
    }else{
      $('.visibility-js').slideDown('100',);
    }
  }); 
  burger.on('click', () =>{
  ...
  let resetTimer = null;
  function arrowOpacity(el){
    setTimeout(function(){
      el.css({
        'opacity': '1',
      });
    });
    resetTimer = setTimeout(function(){el.css({
      'opacity':'0',
  ...
  let scrollAnchor = $('footer');
  function BottomFunction(el){
    let scrollOffset = $(el).offset().top - $(window).height();
    return scrollOffset;
  }  
  $(arrowDown).on('click', () =>{
    $('html, body').animate({ scrollTop: BottomFunction(scrollAnchor)});
  });
  ...
  function chechWidht(){
    ...
    if(windowSize <= 1024-scrollbar){
      ...
      $('.burger').show();
      $('.visibility-js').hide();
      $('.footer-logo-wrapper').append($(gogolisLine));
    }else{
      $('.rekviziti-wrapper').show();
      $('.piedavajumi-wrapper').show();
      $('.mouse-event').hide();
      $('.visibility-js').hide();
      $('.burger').hide();
    }
